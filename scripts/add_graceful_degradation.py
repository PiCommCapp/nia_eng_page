...
def add_graceful_degradation(file_path):
    ...
    if 'serviceWorker' in content and '<!-- Service Worker is optional -->' not in content:
        service_worker_pattern = r'(// Register service worker for offline functionality)'
        service_worker_comment = r'\1\n      // Note: Service worker is optional - site works without it'
